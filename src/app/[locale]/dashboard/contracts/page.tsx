...
export default async function ContractsPage() {
  const t = await getTranslations("dashboard.contractsList");
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-secondary">
                        {employeeNames[doc.employee_id] || t("employee")}
                      </h3>
                      <p className="text-xs text-text-muted">
                        {t("createdOn")} {new Date(doc.created_at).toLocaleDateString("fr-CH")}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-medium px-2.5 py-1 rounded-full text-success bg-success/10">
                      {doc.status === "ready" ? t("statusGenerated") : doc.status}
                    </span>
                    {doc.file_url && (
                      <a
                        href={doc.file_url}
